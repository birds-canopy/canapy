...
def _go_up(directory):
    ...
    directory.param.directory_list.objects = [
        str(d) for d in current_directory.iterdir() if d.is_dir()
    ]
...
class Directory(pn.viewable.Viewer):
    ...
    directory_list = param.ListSelector(
        objects=[str(d) for d in Path.home().iterdir() if d.is_dir()], precedence=0.3
    )
    ...
    def _update_directory_list(self):
        ...
        if current_directory.exists():
            self.param.directory_list.objects = [
                str(d) for d in current_directory.iterdir() if d.is_dir()
            ]
        else:
