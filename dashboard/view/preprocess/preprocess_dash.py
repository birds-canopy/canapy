...
def compute_gated_stats(y, sr, threshold_ratio=0.15):
    """
    Calcule le centroid et la MEAN SLOPE (variation instantanée).
    Cette méthode gère les répétitions (trilles) en calculant la dérivée frame par frame
    et en ignorant les sauts entre les syllabes.
    """
    if len(y) == 0:
        return 0, 0
    ...
    # Besoin d'au moins quelques frames pour calculer une diff
    if np.sum(mask) < 3: 
        return 0, 0
    ...
    # Calcul de la pente moyenne (slope)    
    # 1. Récupérer la fréquence dominante par frame
    freqs = librosa.fft_frequencies(sr=sr, n_fft=N_FFT)
    dom_freq_idx = np.argmax(S, axis=0) 
    dom_freq_hz = freqs[dom_freq_idx]
    
    # 2. Calculer le pas de temps (dt) entre deux frames
    dt = HOP_LEN / sr
    
    # 3. Calculer la différence de fréquence entre frames consécutives (la dérivée)
    # np.diff renvoie [f1-f0, f2-f1, ...]
    delta_freq = np.diff(dom_freq_hz)
    
    # Slope instantanée en Hz/seconde
    instant_slopes = delta_freq / dt
    
    # On ne garde la pente que si la frame N ET la frame N+1 sont actives.
    # Cela permet d'ignorer le saut de fréquence pendant le silence entre deux syllabes.
    ...
    # Dans un trill "\ \ \", le retour vers le haut après avoir atteint le bas est très rapide et souvent un artefact.
    # On enlève les 10% valeurs les plus extrêmes pour nettoyer la moyenne.
...
class PreprocessDashboard(SubDash):
    def __init__(self, parent):
        ...
        self.header = pn.Column(
            ...
            pn.pane.Markdown("Inspect dataset, merge classes, and correct labels.", css_classes=['page-subtitle'], margin=0)
        )
...
class CorrectorView(SubDash):
    ...
    def build_display(self):
        ...
        return pn.Column(
            ...
            pn.Column(self.save_msg, self.save_btn, sizing_mode="stretch_width", margin=(10, 0, 0, 0)),
            sizing_mode="stretch_both"
        )
...
class RepertoireView(SubDash):
    def __init__(self, parent, num_panel, orientation, num_samples=MAX_SAMPLE_DISPLAY):
        ...
        self.select_left = pn.widgets.Select(name="Class A", options=[lbl for lbl in self.controler.classes if lbl != "SIL"], sizing_mode="stretch_width")
        ...
        if num_panel == 2:
            self.select_right = pn.widgets.Select(name="Class B", options=[lbl for lbl in self.controler.classes if lbl != "SIL"], sizing_mode="stretch_width")
    ...
    def build_display(self, label):
        ...
        for i, sp in enumerate(specs):
            visual_block = pn.Row(
                ...
                sizing_mode="fixed", width=320, margin=(0, 10, 0, 0)
            )
            audio_block = pn.Column(
                ...
                sizing_mode="stretch_width", min_width=200, 
            )
            card_content = pn.FlexBox(
                visual_block, audio_block,
                align_items='center', justify_content='start', flex_wrap='wrap', gap=10, sizing_mode="stretch_width"
            )
...
class CorrectionTool(SubDash):
    def __init__(self, parent):
        ...
        self.selector_area = pn.Column(
            ...
            styles={"background": "#f8f9fa", "padding": "15px", "border-radius": "8px", "border": "1px solid #dee2e6"},
        ...
        self.stats_table = pn.widgets.Tabulator(
            ...
            height=200, width=320, sizing_mode="stretch_width",
            configuration={'columnDefaults': {'headerSort': False}}
        )
        
        self.compute_btn = pn.widgets.Button(name="Calculate Stats", button_type="primary", width=160, height=60, align="center")
        self.compute_btn.on_click(self.run_global_stats_calculation)
        self.compute_status = pn.pane.Markdown("<i>Click to compute global stats</i>", styles={'font-size': '11px', 'color': 'grey'})
        
        self.stats_dashboard = self.build_stats_layout()

        self.save_btn = pn.widgets.Button(name="Save all", sizing_mode="stretch_width", button_type="primary", disabled=True)
        self.save_btn.on_click(self.on_click_save)
        ...
        self.layout = pn.Column(
            ...
            pn.Column(
                pn.pane.Markdown("### 2. Correct Samples", margin=(0, 0, 10, 0)),
                self.sample_container,
                sizing_mode="stretch_both",
                min_height=400
            ),
            pn.Column(
                pn.pane.Markdown("### 3. Save corrections", margin=(10, 0, 5, 0)),
                self.save_msg, self.save_btn, sizing_mode="stretch_width"
            ),
            sizing_mode="stretch_both"
        )
    ...
    def build_stats_layout(self):
        ...
        MAX_PER_PAGE = 40
        self.class_chunks = [all_classes[i:i + MAX_PER_PAGE] for i in range(0, len(all_classes), MAX_PER_PAGE)]
        
        self.plot_pane = pn.pane.Matplotlib(tight=True, sizing_mode="stretch_width", min_height=250)
        
        # Sélecteur de statistique pour le boxplot
        ...
        right_panel = pn.Column(
            pn.Row(pn.pane.Markdown("**Class Statistics**", styles={'font-size': '13px', 'font-weight':'bold'}), self.compute_status),
            self.stats_table,
            self.compute_btn,
            width=430, sizing_mode="fixed", margin=(0, 0, 0, 15)
        )
        ...
        return pn.Row(
            ...
            right_panel,
            sizing_mode="stretch_width", css_classes=['stats-container'], margin=(0, 0, 15, 0)
        )
    ...
    def update_boxplot_view(self, event=None):
        ...
        if metric == 'Duration':
            data_to_plot = [temp_df[temp_df['label'] == c]['duration'].values for c in current_classes]
            y_label = "Time (s)"
            ...
            if 'centroid' in temp_df.columns:
                for c in current_classes:
                    vals = temp_df[temp_df['label'] == c]['centroid'].dropna().values
            ...
            if 'slope' in temp_df.columns:
                for c in current_classes:
                    vals = temp_df[temp_df['label'] == c]['slope'].dropna().values
                    data_to_plot.append(vals if len(vals) > 0 else [])
            else:
                data_to_plot = [[] for _ in current_classes]
        ...
        if has_data:
            ax.boxplot(data_to_plot, labels=current_classes, patch_artist=True, 
                       boxprops=dict(facecolor="#3b82f6", color="#1f2937"),
                       medianprops=dict(color="white"))
        else:
            if metric != 'Duration':
                ax.text(0.5, 0.5, "Click 'Calculate Stats' to view data", 
                        ha='center', va='center', transform=ax.transAxes, color='gray')
            ax.set_xticklabels(current_classes)
        
        title_suffix = f" (Page {self.plot_pager.value}/{len(self.class_chunks)})" if len(self.class_chunks) > 1 else ""
    ...
    def run_global_stats_calculation(self, event):
        ...
        # Initialisation des colonnes de stats
        ...
        try:
            if 'duration' not in self.controler.corpus.dataset.columns:
                self.controler.corpus.dataset['duration'] = self.controler.corpus.dataset['offset_s'] - self.controler.corpus.dataset['onset_s']

            for c in classes:
                full_class_df = self.controler.corpus.dataset.query("label == @c")
                count = len(full_class_df)
                avg_dur = full_class_df['duration'].mean() if not full_class_df.empty else 0
                ...
                if not sub_df.empty:
                    try:
                        ...
                        for (idx, row), item in zip(sub_df.iterrows(), repertoire):
                            audio_data = item[1]
                            ...
                            if len(audio_float) > 0:
                                gated_cent, gated_slope = compute_gated_stats(audio_float, sampling_rate)
                                
                                if gated_cent > 0: 
                                    centroid_list.append(gated_cent)
                                    self.controler.corpus.dataset.at[idx, 'centroid'] = gated_cent
                                    # La slope peut être 0 ou négative, on l'enregistre si le centroid est valide
                                    slope_list.append(gated_slope)
                                    self.controler.corpus.dataset.at[idx, 'slope'] = gated_slope
                                
                    except Exception as e:
                        logger.error(f"Stats Error Class {c}: {e}")

                avg_centroid = int(np.mean(centroid_list)) if centroid_list else 0
    ...
    def listen_class_selection(self, label):
        ...
        self.sample_container.height = 550
...
class ClassSelectionView(SubDash):
    def __init__(self, label, total_count, parent):
        ...
        self.select_btn = pn.widgets.Button(name=label, button_type="default", height=35, sizing_mode="stretch_width")
        ...
        self.corrected_msg = pn.pane.Markdown(
            ...
            styles={"text-align": "center", "font-size": "11px", "color": "#6b7280"}, margin=(5, 0)
        )
        self.layout = pn.Column(self.select_btn, self.corrected_msg, width=110, css_classes=['selector-card'])
...
class SampleListView(SubDash):
    ...
    def build_display(self):
        ...
        for i, (segment, (idx, row)) in enumerate(zip(segments, self.sliced_df.iterrows())):
            display_num = i + 1
            if self.registry.get(idx) is None:
                display = SingleSampleRow(row, segment, self.parent, idx, display_num)
