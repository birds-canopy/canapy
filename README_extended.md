...
* The **sampling rate** is automatically detected from your audio files. You can override it manually if needed. If the *Downsample* option is enabled, audio will be resampled to the configured sampling rate at load time.
...
Before starting training, you can optionally run an automatic **hyperparameter (HP) search** to find better ESN parameters for your dataset. This search uses the **TPE** (Tree-structured Parzen Estimator) algorithm for sequential optimization, or **parallel random search** for faster exploration with multiple CPU cores.

Key settings (configurable on the **Settings** page):
- **opt_max_evals**: maximum number of HP configurations to evaluate.
- **opt_n_jobs**: number of parallel evaluation jobs (1 = sequential TPE, >1 = parallel random).
- **opt_max_percentage**: fraction of the training data used during each HP evaluation (to speed up the search).
...
The **Settings** page lets you view and modify the configuration parameters used by Canapy. Parameters are grouped into three categories:

#### Species parameters
These control the spectrogram computation and define the frequency range of interest for the species:
- **fmin** / **fmax**: minimum and maximum frequencies (Hz) used to crop the spectrogram.
- **win_length**: STFT window length (in samples).
- **hop_length**: hop size between successive STFT windows (in samples).
- **n_fft**: FFT size.

#### ESN parameters
These control the Echo State Network architecture:
- **sr** (spectral radius): controls the memory of the reservoir — higher values increase temporal integration.
- **leak**: leak rate of the reservoir neurons.
- **iss**: input scaling for the main input connection.
- **isd** / **isd2**: input scaling for additional dense input connections.
- **ridge**: regularization coefficient for the readout (higher values = more regularization).

#### HP search parameters
These control the automatic hyperparameter optimization (see **Hyperparameter search** above):
- **opt_max_evals**, **opt_n_jobs**, **opt_max_percentage** as described in the training section.

After modifying parameters, click **Validate** to apply them to the current session.
