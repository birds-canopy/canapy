...
from canapy.annotator.commons.postprocess import frames_to_timed_df, frames_to_seconds, frame_df_to_annots_df


def test_frames_to_timed_df(cls_frame_predictions):
    df = frames_to_timed_df(cls_frame_predictions, notated_path="foo/baz", frame_size=256, sampling_rate=44100)
    print(df)


def test_frame_df_to_annots_df(cls_frame_predictions):

    df = frames_to_timed_df(cls_frame_predictions, notated_path="foo/baz", frame_size=100, sampling_rate=10000)
    print(df)
